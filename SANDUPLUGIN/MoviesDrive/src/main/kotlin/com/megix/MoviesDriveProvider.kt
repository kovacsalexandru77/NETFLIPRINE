...
class MoviesDriveProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        // Lekéri az iframe linkeket a videó URL-ből
        val iframeSrc = Jsoup.connect(data).get()
            .select("iframe[src*='videa'], iframe[src*='ok.ru'], iframe[src*='vk.com']")
            .attr("src")
        ...
        if (iframeSrc.isNotEmpty()) {
            callback(
                ExtractorLink(
                    ...
                    isM3u8 = iframeSrc.endsWith(".m3u8")
                )
            )
        } else {
            // Ha nem talál iframe-t, akkor hibaüzenet
            println("Hiba: Nem található iframe forrás.")
        }
    }
}
